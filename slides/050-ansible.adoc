...
[.notes]
****
Now, let's talk about the solution I've chose. Ansible is a python automation tool to provision and configure server. It can act on hundred of distant servers to apply configuration or, what will be interesting for us, one local workstation.

Ansible has a notion of roles and playbook we'll talk about in the next slides, so we can separate/regroup our tasks. It also have a huge community we can rely on to use existing roles to configure your docker installation as example.

Whether we like it or hate it, it is easy to read, using a YAML syntax.
****
...
[.notes]
****
So I've told about the solution I've chosen. But what about how to use it ? We'll see the structure of an Ansible playbooks/roles and how I use it to provision my workstation.
****
...
[.notes]
****
First, if we look at this filetree, we can see that playbook are organized in various folders. Roles folder contains different local roles, to achieve related tasks, such as anything concerning my git configuration (key signing, global configuration...) or install the jetbrains toolbox.

Then we can find the playbook directory that contains one folder per workstation, and each playbook include a combinaison of local roles and community roles, that once put together, ensure my workstation is up to date.
...
[.notes]
****
Now, what concretely a playbook look like ? It is a YAML file, that run on a host or a set of hosts. In my case, I only want to configure my local workstation, so I run it on localhost.

Then we have a array of 'tasks' key that contains different tasks. A tasks should always be named after what it does, and call a module, in this example the "debug" module with optional attributes depending on the called module. Here, we can see that I call my debug module with a msg attribute.

We then see an array of roles, that can either come from a local role, such as my git_config we've seen before, a from the community, such as the docker role. This means that you could share your own role to perform a dedicated task or set of tasks and share it with the community
...
[.notes]
****
Now let's see how to apply this configuration on my workstation. First, we'll need to install the Python dependencies. Since Python is wildly available, on macOS and Linux, it should be available out of the box and you can easily install it on Windows too. We can see on the requirements.txt that there is only dependency, ansible itself.

Then we can install the community related roles stored on ansible galaxy. To do so, we can declare these in a requirements.yml file that contains an array of roles and their associated versions, here the 6.1 version for docker.
****
...
[.notes]
****
Our dependencies are installed, we are ready to go and can run ansible to configure our workstation and ensure to keep it up to date !
...
The first one is the "package" module to install software. We give to this module a list of desired software, and ansible will ensure these package are installed on this workstation. Obviously, this requires administrator privileges. One interesting argument we can give to this module is the "state". By default, it ensure that software is present on the workstation, but we can also define a task where we want to ensure that software is absent. This can be useful if you want to uninstall default software that you don't use.

You can use variable to install different software on your personal and professional workstation. To do so, one way is to override the variable value, here "package_to_install", defined in the role folder.
...
[.notes]
****
First, we need to create a vault where we'll store our secret. It can be sensitive variable or full encrypted configuration files. Ansible-vault will ask for a password to symmetrically encrypt our file with this password. To decrypt it, we can use the view option of ansible vault.
...
video::personal.mp4[width=1000,opts="autoplay,loop,muted"]
