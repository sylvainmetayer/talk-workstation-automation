...
[.notes]
****
Now, let's discuss the solution I've chosen. Ansible is a python automation tool to provision and configure servers. It can be used to apply configuration on hundreds of distant servers  or, what will be interesting for us, one local laptop.
...
[.notes]
****
Let's see how to apply this configuration on my laptop. First, we'll need to install the Python dependencies.
...
[.notes]
****
Now, let's see a few ansible modules that can come in handy to configure our laptop.

The first one is the "package" module to install software. We give the module a list of the software we want, using a variable, here containing vim, firefox and vscode by default, and ansible will ensure these package are installed on this laptop. Obviously, this requires administrator privileges.

You can use variables to install different software on your personal and professional laptop. To do so, one way is to override the "package_to_install", defined in the role folder.

Let's assume that I don't need vscode on my personal laptop. I will therefore override the variable value in my personal playbook to only install firefox and vim
****
...
[.notes]
****
Now, we sometime need to copy secrets in a dedicated location in our laptop, like a token value to authenticate against a private registry.
...
[.notes]
****
As we saw before, a playbook is a combination of roles and tasks with custom variables. Therefore, to manage multiple laptop, we just have to create a new playbook and use needed roles and/or override variable values.
****
...
[.notes]
****
To put it in a nutshell, ansible has my back on all of my needs, with very few prerequisites. I'm able to manage my dotfiles and installed software, in an easy way with YAML syntax. I can also review changes quite easily and adding a new software or configuration file doesn't take much time. I'm able to manage my personal and professional laptop with dedicated playbooks, reusing roles for most of my needs. I'm also able to handle secrets with the vault system to store my secrets safely.
