...
[.notes]
****
Now, let's discuss the solution I've chosen. Ansible is a python automation tool to provision and configure servers. It can be used to apply configuration on hundreds of distant servers  or, what will be interesting for us, one local workstation.
...
[.notes]
****
Now, let's see a few ansible modules that can come in handy to configure our workstation.

The first one is the "package" module to install software. We give the module a list of the software we want, using a variable, here containing vim, firefox and vscode by default, and ansible will ensure these package are installed on this workstation. Obviously, this requires administrator privileges.

You can use variables to install different software on your personal and professional workstation. To do so, one way is to override the "package_to_install", defined in the role folder.

Let's assume that I need vscode, firefox and vim on my professional workstation. I don't need to override its value and will use the default one. However, on my personal workstation, I don't need vscode and will therefore override the variable value in my personal playbook to only install firefox and vim
...
[.notes]
****
This role reproduces the behavior of stow by creating symlinks of a set of files in my home directory. We can see that I have a symlink role that contains a folder named files, where I store all my dotfiles configuration.
...
[.notes]
****
Sadly, symlinks are not always an option, you may have some files that require dedicated configuration to be functional. A classic example is my gitconfig file, where I have my name/email that is different on my professional and personal workstation. Thankfully, Ansible offers a templating system with loops, variables and many others options, using Jinja2 as template engine, so we can use it to template our file with custom values.
...
[.notes]
****
As we saw before, a playbook is a combination of roles and tasks with custom variables. Therefore, to manage multiple workstations, we just have to create a new playbook and use needed roles and/or override variable values.
****
...
[.notes]
****
To put it in a nutshell, ansible has my back on all of my needs, with very few prerequisites. I'm able to manage my dotfiles and installed software, in an easy way with YAML syntax. I can also review changes quite easily and adding a new software or configuration file doesn't take much time. I'm able to manage my personal and professional workstation with dedicated playbooks, reusing roles for most of my needs. I'm also able to handle secrets with the vault system to store my secrets safely.
